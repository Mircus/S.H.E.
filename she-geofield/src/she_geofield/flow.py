...
def fixed_geometry_step(complex_: WeightedToyComplex, x1: np.ndarray, dt: float = 0.5) -> np.ndarray:
    L1 = hodge_laplacian_1(complex_)
    P = expm(-dt * L1)
    return P @ x1

def geometry_step(complex_: WeightedToyComplex, x1: np.ndarray = None,
                  eta: float = 0.1, lam: float = 0.5, mu: float = 0.0) -> None:
    ...
    for tri in complex_.triangles:
        F = triangle_curvature(complex_, tri, x1=x1, lam=lam, mu=mu)
...
def coupled_step(complex_: WeightedToyComplex, x1: np.ndarray,
                 ...
                 mu: float = 0.0):
    x_next = fixed_geometry_step(complex_, x1, dt=dt)
    geometry_step(complex_, x1=x1, eta=eta, lam=lam, mu=mu)
    return x_next
...
def iterate_coupled(complex_: WeightedToyComplex, x1: np.ndarray,
    ...
    for _ in range(steps):
        cur = coupled_step(complex_, cur, dt=dt, eta=eta, lam=lam, mu=mu)
